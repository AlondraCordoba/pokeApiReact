...
import {
  ...
  BuscadorConsumer,
  Result,
} from "./components/context/contextSearch";
      ...
      pokemonInfo.slice(numberOne, numberTwo).filter((elemento) => {
        if (
          elemento.name
            .toString()
            .toLowerCase()
            .includes(info.search)
        ) {
          setArray([]);
          const url = elemento.url.split("/");
          axios
            .get(`https://pokeapi.co/api/v2/pokemon/${url[6]}`)
            .then((data) => {
          ...
          <div>
                  ...
                  <ThemeProvider key={index} theme={theme}>
                    <Tooltip title={info.data.name} placement="top">
                      <img
                        ...
                        src={
                          side === true
                            ? info.data.sprites.back_default
                              ? shiny === true
                                ? info.data.sprites.back_shiny
                                : info.data.sprites.back_default
                              : "https://i.pinimg.com/originals/ef/72/4f/ef724f2c2cf02a434b8464f17fe40ca1.gif"
                            : info.data.sprites.front_default
                            ? shiny === true
                              ? info.data.sprites.front_shiny
                              : info.data.sprites.front_default
                            : "https://i.pinimg.com/originals/ef/72/4f/ef724f2c2cf02a434b8464f17fe40ca1.gif"
                        }
                        alt="Not found"
            ...
            <div className="center-a">
              <button
                className="btn-left"
                onClick={(e) => {
                  setNumberOne(numberOne - 10);
                  setNumberTwo(numberTwo - 10);
                }}
              ></button>
              <button
                className="btn-right"
            ...
            <div className="center-a">
              <Stack direction="row" spacing={2}>
                <Button color="secondary" className="btn-info" ref={message} onClick={handleChange}>
                <svg
